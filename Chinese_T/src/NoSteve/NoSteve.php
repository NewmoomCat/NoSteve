...
class NoSteve extends PluginBase implements Listener
{
    ...
    public function onPlayerPreLogin(PlayerPreLoginEvent $event)
    {
        ...
        {//防止某些人用纳粹和希特勒取名
            $event->setCancelled();
            $event->setKickMessage(TextFormat::RED."此用戶名攝政敏感\n\n出錯了");
        }
        ...
        {//辱华名字
            $event->setCancelled();
            $event->setKickMessage(TextFormat::RED."你在辱華\n\n出錯了");
        }

        if (strtolower($event->getPlayer()->getName()) === '731')
        {//每个中国人的伤痛
            $event->setCancelled();
            $event->setKickMessage(TextFormat::RED."中國人的傷痛\n\n出錯了");
        }

        if (strtolower($event->getPlayer()->getName()) === 'nigger')
        {//种族歧视
            $event->setCancelled();
            $event->setKickMessage(TextFormat::RED."種族歧視\n\n出錯了");
        }
    }
}